...
1. Install [uv](https://docs.astral.sh/uv/getting-started/installation/), then sync the project environment (a CUDA-capable GPU is recommended for training):
...
```
uv run python get_results.py model_results.csv
```
...
```
uv run python -u cancer_detector.py --arch densenet121 --ensemble 3
uv run python -u cancer_detector.py --task melanoma
uv run python -u cancer_detector.py --eval-only --output model_results.csv
```
